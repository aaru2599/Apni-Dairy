...
const CartLayout = () => {
  ...
  const onClickProductRemove = (productId) => {
    ...
    const isCoonfirm = confirm(
      `Do you want to remove ${removeProduct.pName} product`
    );
    if (isCoonfirm) {
  ...
  return (
    <div className="main-div"  style={{backgroundColor:"#e9e9e9"}}>
      ...
      <div className="d-flex justify-content-center">
        <div className="text-center  w-10 bg-light fs-2   rounded  my-3">
          Cart
        </div>
      </div>
      {cartData.length > 0 ? (
                    ...
                    <tr key={product.pId}>
                      <td>
                        ...
                        <div className="d-flex gap-4">
                          <div>
                            <img
                              className="border bg-light object-fit-contain"
                              src={product.pImage}
                              width={100}
                              height={100}
                              alt=""
                          ...
                          <div className="w-75 d-flex flex-column gap-3">
                            <div className="d-flex justify-content-between flex-column gap-2">
                              ...
                              <div>
                                <span className="border  bg-$red-300 px-1 rounded">
                                  Size: {product.count}
                                </span>
                              </div>
                            </div>
                            <div className="d-flex justify-content-between">
                              ...
                              <div>
                                <button
                                  className=" btn  bi bi-trash-fill text-body-secondary "
                                  onClick={() =>
                                    onClickProductRemove(product.pId)
                                  }
                                ></button>
                                <button
                                  className=" btn  bi bi-dash-circle fw-bold"
                                  onClick={() => onClickDecrease(product.pId)}
                                ></button>
                                <span className="text-body-secondary">
                                  {product.count}
                                </span>
                                <button
                                  className=" btn bi bi-plus-circle"
                                  onClick={() => onClickIncrease(product.pId)}
                                ></button>
                              </div>
                            </div>
                          </div>
                ...
                <tbody>
                        ...
                        {cartData.map((item) => {
                          return (
                            <div
                              ...
                              key={item.pId}
                            >
                              <div className="text-truncate w-80">
                  ...
                  <tr>
                    <td>
                      ...
                      <div>
                        <div>
                          <span className="text-secondary bi bi-truck"></span>{" "}
                          <span className="mx-1">Ecom Express</span>
                        </div>
                        <div>
        ...
        <div className="text-center">
          ...
          <div>
            <Link to="/products">Continue Sopping</Link>
          </div>
        </div>
      )}
      <ToastContainer />
